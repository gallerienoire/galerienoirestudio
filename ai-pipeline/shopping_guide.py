        ...
        "floor_lamp": {
            "tiers": {
                "luxury": {
                    ...
                    "note": "Sculptural presence that doubles as art. The brass finish ties to the room's accents."
                },
                "premium": {
                ...
                "budget": {
                    ...
                    "retailer": "IKEA",
                    "price_range": "$80 - $150",
                    "note": "Classic adjustable design. Spray-paint the base in brass for a custom look."
                }
            },
            "recommended_tier": "premium",
...
if __name__ == "__main__":
    """CLI entry point for standalone shopping guide."""
    import sys
    
    if len(sys.argv) < 2:
        print("Usage: python shopping_guide.py <room_profile.json> [--json]")
        sys.exit(1)
    ...
    result = generate_shopping_guide(ctx, quiet=quiet)
    
    if quiet:
        print(json.dumps(result, indent=2))
    else:
        print("\n" + "=" * 60)
        print("SHOPPING GUIDE (JSON)")
        print("=" * 60)
        print(json.dumps(result, indent=2))
